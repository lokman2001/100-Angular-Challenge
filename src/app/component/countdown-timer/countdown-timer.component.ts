...
export class CountdownTimerComponent {
  ...
  get sec(): string {
    ...
    const secValue = Math.floor((this.getTimeDifference() % (miliSec * sec)) / miliSec);
    return this.timeFormat(secValue);
  }
  get min(): string {
    const { miliSec, sec, min } = this.timerNumeric;
    const minValue = Math.floor((this.getTimeDifference() % (miliSec * sec * min)) / (miliSec * sec));
    return this.timeFormat(minValue);
  }
  get hour(): string {
    const { miliSec, sec, min, hour } = this.timerNumeric;
    const hourValue = Math.floor((this.getTimeDifference() % (miliSec * sec * min * hour)) / (miliSec * sec * min));
    return this.timeFormat(hourValue);
  }
  get days(): string {
    const { miliSec, sec, min, hour } = this.timerNumeric;
    const daysValue = Math.floor(this.getTimeDifference() / (miliSec * sec * min * hour));
